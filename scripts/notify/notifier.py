"""Build-Eye 通知模块 - 支持飞书应用消息、钉钉企业消息、邮件通知，均支持多收件人。"""
import hashlib
import hmac
import base64
import json
...
def _feishu_get_tenant_token(app_id: str, app_secret: str) -> Optional[str]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as resp:
            ...
            print(f"飞书获取 tenant_access_token 失败: {result.get('msg')}")
            return None
    except Exception as e:
        print(f"飞书获取 token 请求失败: {e}")
        return None
    ...
    for receive_id in receive_ids:
        ...
        try:
            with urllib.request.urlopen(req, timeout=10) as resp:
                ...
                if not ok:
                    print(f"飞书发送给 {receive_id} 失败: {result.get('msg')}")
                results[receive_id] = ok
        except Exception as e:
            print(f"飞书发送给 {receive_id} 请求失败: {e}")
...
def _dingtalk_get_token(app_key: str, app_secret: str) -> Optional[str]:
    ...
    try:
        with urllib.request.urlopen(url, timeout=10) as resp:
            ...
            print(f"钉钉获取 access_token 失败: {result.get('errmsg')}")
            return None
    except Exception as e:
        print(f"钉钉获取 token 请求失败: {e}")
        return None
    ...
    for user_id in receive_ids:
        ...
        try:
            with urllib.request.urlopen(req, timeout=10) as resp:
                ...
                if not ok:
                    print(f"钉钉发送给 {user_id} 失败: {result.get('errmsg')}")
                results[user_id] = ok
        except Exception as e:
            print(f"钉钉发送给 {user_id} 请求失败: {e}")
